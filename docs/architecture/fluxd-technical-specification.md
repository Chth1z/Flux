...
Implementation checkpoint: the current substrate implements canonical address snapshots, strict address decoding, AF_UNSPEC address dumps, loss/debounce/resynchronization state, and the subscribed route-netlink socket. This is not yet the production Adapter described above: link/route/rule fact codecs, the opaque socket/decoder/observer driver, and single-reactor registration remain required before the Phase 3 observer is complete.
