...
Its current first slice adds a third probe network namespace and sends dual-stack TCP echo through
exact PREROUTING TPROXY selectors into a test-local transparent Rust relay. It proves accepted
socket original-destination recovery, relay and peer tuples, relay socket-mark readback,
per-family route controls, bounded capture/bypass counters, and exact cleanup. UDP echo and DNS
over UDP/TCP remain the next extension under the same command and exact test. Before invoking
...
Distinct nonzero probe/engine UIDs, exact local-OUTPUT listener delivery, complete `/proc` FD plus
INET_DIAG correlation (or separately qualified cgroup-eBPF correlation), and the model
`validate_for` call remain required before Linux evidence can represent the complete transaction.
