...
Current implementation status: the committed observer foundation has one canonical link/address inventory, strict private rtnetlink decoders, and a subscribed nonblocking socket that performs one sequential `RTM_GETLINK` then `AF_UNSPEC RTM_GETADDR` transaction. LINK completion retains its facts without publishing; ADDRESS completion replays transaction-wide bounded races and publishes one combined `NetworkInventorySource` epoch. Link decoding preserves raw names and link kinds through the netlink wire bound, unknown flags/types/states, and extended dump acknowledgements while rejecting ambiguous or loss-marked datagrams; the observer preserves optional fields omitted by partial live notifications. The driver uses 256 KiB receive slots, a 1 MiB default per-turn byte budget, fresh phase deadlines, exact sequence ownership, and optional registration in the daemon's existing reactor after capability admission.

The route foundation adds canonical route domain facts and a strict private `RTM_NEWROUTE`/`RTM_DELROUTE` decoder, including canonical prefixes, raw table/protocol/scope/type/flag preservation, direct and cross-family-via gateways, ordered multipath weights, named-nexthop IDs, `NLM_F_REPLACE`, and strict loss/DONE/attribute validation. It deliberately does not extend the socket transaction or `NetworkInventory`: metrics, encapsulation, flow, and new-destination semantics remain a lossy topology/selection projection; NH-ID-only paths require later nexthop-object observation or compatibility gating; and live route identity/replacement is not yet defined.
