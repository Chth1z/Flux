...
Current implementation status: the committed foundation now has one canonical link/address inventory, strict private rtnetlink decoders, and a subscribed nonblocking socket that performs one sequential `RTM_GETLINK` then `AF_UNSPEC RTM_GETADDR` transaction. LINK completion retains its facts without publishing; ADDRESS completion replays transaction-wide bounded races and publishes one combined `NetworkInventorySource` epoch. Link decoding preserves raw names and link kinds through the netlink wire bound, unknown flags/types/states, and extended dump acknowledgements while rejecting ambiguous or loss-marked datagrams; the observer preserves optional fields omitted by partial live notifications. The driver uses 256 KiB receive slots, a 1 MiB default per-turn byte budget, fresh phase deadlines, exact sequence ownership, and optional registration in the daemon's existing reactor after capability admission. It remains an incomplete Phase 3 tracer bullet: canonical route/rule facts and native policy-routing mutation are still pending.
