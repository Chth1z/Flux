...
are a real local-OUTPUT capture receipt, schema-v2 listener-observer/report factories, production
context use of the delivered attempt-owned outbound-collector handoff, capability-qualified
engine/probe execution, and Android adapter/qualification. The validator itself is complete and rejects weak,
substituted, lossy,
...
2. Continue the positive local-OUTPUT producer on the delivered fail-closed TPROXY-only seam and
   exact credential preflight. First define an explicit local-OUTPUT capture-receipt contract;
   then add exact probe and engine UID+GID/process observations, real listener-socket and delivery-
   report factories, and the exact attempt-owned supervised report schema/object identity or a
