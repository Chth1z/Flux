...
- Added the first Stage-2 privileged Linux namespace checkpoint and `cargo xtask test-functional-canary-linux`: an isolated, journaled topology exercises real IPv4/IPv6 TCP, UDP, DNS-over-UDP, and DNS-over-TCP flows with independent client/peer evidence and exact cleanup. This checkpoint is topology-only; TPROXY traversal, distinct-UID loop escape, counters, INET_DIAG correlation, and complete model validation remain pending there.
- Added `cargo xtask test-functional-canary-linux-tproxy` and the first ingress-only TPROXY slice: a third probe namespace now exercises real IPv4/IPv6 TCP PREROUTING capture, transparent original-destination recovery, test-local marked relay egress, route controls, bounded counters, and exact cleanup. UDP/DNS relay coverage, local-OUTPUT qualification, distinct UIDs, socket correlation, complete model validation, and Android evidence remain pending.
