...
TPROXY-only local-OUTPUT executor and capture-receipt boundaries are also delivered: read-only
typed availability is separate from prepared execution, drivers return unverified capture proof
and raw observations, a sealed verifier alone may mint a non-cloneable per-flow receipt, and only
receipt-bound artifacts reach the module-private evidence factory. The current zero-state xtables
...
uninhabited, so it adds no positive traffic or evidence path. Still deferred are attempt-owned
UID/GID/PID/start-tick/handle binding, schema-v2 listener-observer/report parsing and factories,
...
2. Continue the positive local-OUTPUT producer on the delivered fail-closed TPROXY-only seam,
   exact credential preflight, and completed per-flow capture-receipt/verifier contract. Add exact
   attempt-owned probe and engine UID+GID/PID/start-tick/handle observations, real listener-socket
   ...
   cleanup process identity to an attempt-owned process handle, and use the typed handoff from the
   real attempt context to carry the prebound strict `/proc` FD plus INET_DIAG session whose port
