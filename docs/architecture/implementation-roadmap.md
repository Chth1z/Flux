...
- **Delivered model and wire boundary:** add exact Android product/build/vendor/security-patch,
  kernel-build, verified-boot/vbmeta, SELinux-policy, netd/Connectivity artifact, bounded tool, boot,
  and namespace identity to the freshness-bound profile. The reviewed Android collector remains
  pending and the generic source reports `Unavailable`.
