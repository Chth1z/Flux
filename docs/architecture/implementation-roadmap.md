...
2. Extend the delivered `cargo xtask test-functional-canary-linux-tproxy` checkpoint, which now covers dual-stack TCP/UDP echo under `functional_canary::linux_namespace_harness::privileged_ingress_tproxy_checkpoint_exercises_real_capture_counters_and_cleanup`, with DNS over UDP/TCP. Preserve exact original-destination/source behavior, per-family route controls, independent bounded protocol counters, cleanup, and the regression that forbids TPROXY in OUTPUT.
