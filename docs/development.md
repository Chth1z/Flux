...
slice proves ingress PREROUTING TPROXY, accepted-socket and strict ancillary-data
original-destination recovery, marked relay egress, source-preserving UDP replies, per-family route
controls, independent bounded TCP/UDP capture/bypass counters, and cleanup. DNS over UDP/TCP
remains the next extension under the same command and exact test. The deterministic
