...
- Corrected the earlier kernel-wide interpretation of the local-OUTPUT experiment. Linux 5.10 can
  ...
  candidate while keeping the production driver fail-closed until the full mark, route, listener,
  escape, ownership, cleanup, and Android-qualification contract is implemented.
- Added `cargo xtask test-functional-canary-linux-output-tproxy`, an opt-in, mechanism-only
  ...
  preflight, construct Generation/canary authority, run a production report producer, or qualify a
  production Android profile.
- Added `cargo xtask test-functional-canary-android-x86_64-output-tproxy --serial SERIAL
  [--adb PROGRAM]`, a non-shipping Rust runner that cross-builds the exact ignored checkpoint,
  selects one explicit rooted x86_64 Android device, binds its fingerprint and boot ID across the
  build/execution/cleanup boundaries, bounds every host command, uses a private `/data/local/tmp`
  `TMPDIR`, forces required mode, and proves remote cleanup. The checkpoint passed on WSA Android
  13 / SDK 33 with Magisk root, SELinux enforcing, legacy iptables, and kernel 5.15.104. The Android
  path preserves netd-owned socket-mark bits through masked merge and handles bounded built-in-
  kernel, legacy-iproute2, negative-drop, and inactive-loopback differences. This is profile-
  specific mechanism evidence only; production driver/receipts, distinct UID, Android
  5.10/ARM64, VPN/netd coexistence, crash recovery, packaging, and release qualification remain
  open.

### Canonical xtables lowering
- Extended the pure canonical lowerer with a schema-v2 representation of the complete ADR-0012
  local-OUTPUT transaction while preserving exact schema-v1 forwarded-only bytes and digests.
  Generation-scoped `O` chains render ordered direct decisions followed by protocol-qualified
  masked `MARK`; mark-qualified loopback `P` chains render the exact-port TCP/UDP TPROXY companion;
  existing forwarded `F` chains remain available for mixed programs. Entry-point metadata binds
  hook roles and selectors, and typed requirements bind the transparent listener, engine loop
  escape, per-family RPDB/local-route identity, prepare/attach/detach/retire order, digests, and
  resource budgets. The restore grammar now seals all three private chain roles.
- Forwarded whole-set interface selectors and local UID allowlists expand as positive proxy
  membership, avoiding incorrect negated-rule algebra. Established-flow caching, transparent-socket
  DIVERT, FakeIP ICMP, QUIC rejection, and MSS clamping remain explicit unsupported extensions. The
  artifacts create and remove only unattached private chains: they do not mutate built-in hooks,
  invoke restore, provision routes/listeners, inspect live state, prove cleanup invertibility,
...
- Added a TPROXY-only local-OUTPUT executor/driver/evidence-factory seam. Non-TPROXY requests fail as invalid evidence before driver preparation; pre-mutation availability maps to cleanup `NotRequired`; and post-preparation failures cannot claim that cleanup was unnecessary. At this earlier checkpoint, the zero-state xtables driver reported `Unsupported` before mutation because the then-current canonical lowering and production driver did not implement or authorize the complete OUTPUT-mark, RPDB local-route, loopback-PREROUTING-TPROXY, listener, escape, and cleanup transaction; its prepared/raw types were uninhabited, so no positive traffic or evidence path existed. This is fail-closed evidence admission, not a change to the user-selected fail-open connectivity policy; production remains `structural_only` pending the real attempt context, observer/report factories, capability-qualified execution, and Android qualification.
