...
The initial `flux-platform` xtables checkpoint is only a frozen-syntax observer. A pure parser
...
framing. This parses neither shell configuration nor Capture Policy and proves neither option
semantics, backend support, kernel acceptance, apply/cleanup invertibility, full cleanup coverage,
nor parity with shell-generated bytes.
...
The raw shell/AWK oracle is a separate cache-artifact checkpoint. Its reviewed provenance starts
at `docker.io/library/busybox:1.37.0-uclibc@sha256:39e0df8c4d65953b55c344f017e1ff2e0031a7454b3c24e6b76d402f207e315a` and selects the exact
`linux/amd64` OCI manifest
`docker.io/library/busybox@sha256:dfb66b2b3e6981fefa54fd2cd4faf662c35b4a4baeff48295a9409ddf3224c48`,
whose `/bin/busybox` SHA-256 is
`c984eacc3b736fe1eeefe201f21b241932ef4c3c03fbb6869a4f156f32dd9716`. The manifest also pins
the sole layer and version banner. The runner requires this image to be present, selects
`linux/amd64`, disables pulling and container networking, drops all capabilities, keeps the image
root read-only, and streams only the exact bounded oracle-input snapshot into private tmpfs. It
executes only the semantic shell check plus bounded fixture generator; it does not mount the host
workspace, invoke an xtables restore executable, or inspect or mutate live networking state.

`cargo xtask xtables-oracle --check` regenerates and compares exactly four raw fixtures: IPv4
apply, IPv4 cleanup, IPv6 apply, and IPv6 cleanup. The explicit
`cargo xtask xtables-oracle --update` path rewrites them and their reviewed manifest metadata after
an intentional input change, but cannot alter or bless the compile-time approved environment pins.
Normal `cargo xtask ci` never invokes either mode. A distinct GitHub Actions job performs an
explicit pull of the exact platform digest and then runs `--check`; the oracle runner itself
remains networkless and uses `--pull=never`.
...
`SystemSocketDiagnosticsSource::collect_until` remains a compatibility wrapper that opens one
temporary session and performs one collection. Successful stateful and compatibility snapshots
retain the same complete PID/start-tick, stable pre/post FD inventory, four dump, bounds, tuple,
cookie, mark, and timing guarantees.
