...
The implementation is pure computation. It hides normalization, policy ordering, mark allocation, UID expansion, CIDR canonicalization, Sing-Box overlay generation, backend scoring, resource budgeting, and safety validation.
...
- Allocate configurable, validated priority and mark ranges only after observing Android-owned state.
...
- Re-observe after netd or default-network changes.
...
The Phase 1 Supervisor separates two proofs that the completed Controller will compose. Descriptor-pinned validation of the exact binary, configuration, and optional launcher plus child-owned listener/TUN evidence is the pre-capture admission proof; it is not by itself a functional traffic or loop-prevention proof. The runtime handoff publishes capture only after that admission succeeds, then performs the profile-specific functional probe. A failed post-capture probe detaches capture before the candidate is retired and never promotes the service to `RUNNING`.
...
- Property-based tests for normalization, policy ordering, CIDR sets, mark allocation, and idempotence.
