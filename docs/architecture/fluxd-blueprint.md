...
- Select production policy assertions only from a compile-time reviewed catalog keyed by stable Android product/build/vendor, kernel-build, SELinux-policy, and netd/Connectivity artifact identities. The selected entry supplies the only allowed netd profile for RPDB/topology classification. Then freshness-bind the assertion to the full Capability Profile, including executing-tool identity, verified boot, boot ID, and observed network namespace. Runtime-only boot/namespace/tool identities are not compile-time keys, and a runtime manifest does not become trusted by hashing its own bytes (ADR-0014).
