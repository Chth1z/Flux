...
Implementation checkpoint: the current substrate implements canonical address snapshots, bounded canonical link fact types, strict private address and link decoders, AF_UNSPEC address dumps, loss/debounce/resynchronization state, the subscribed route-netlink socket, and an opaque hard-budgeted driver registered with the existing daemon reactor only after the Capability Profile admits the mutation-capable startup path. The link decoder preserves raw names and link kinds through the netlink wire bound, unknown flags/types/states, extended dump acknowledgements, and whole-datagram loss semantics, but runtime `NetworkInventorySource` publication remains address-only. Permanent observer failures invalidate the source and disable only observation. This is not yet the complete production Adapter described above because coordinated link/address dumps and canonical route/rule fact codecs remain pending.
