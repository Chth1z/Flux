...
`0x7fe0_0000` device-qualified candidate envelope. The socket cell records a low-16-bit
`PredicateRead` and merged `MaskedWrite`; packet/conntrack copy operations remain assigned to the
separate `ConnmarkAndSocketTransfers` source. Packet and socket coverage are complete-present,
...
complete census. Together with RPDB it models six cells; the remaining 21 cells and cross-source
point-in-time coordination remain deferred.
