...
- Added the first Stage-2 privileged Linux namespace checkpoint and `cargo xtask test-functional-canary-linux`: an isolated, journaled topology exercises real IPv4/IPv6 TCP, UDP, DNS-over-UDP, and DNS-over-TCP flows with independent client/peer evidence and exact cleanup. TPROXY traversal, distinct-UID loop escape, counters, INET_DIAG correlation, and complete model validation remain pending.
