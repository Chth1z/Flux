...
Implementation checkpoint: the current substrate implements canonical address snapshots, strict address decoding, AF_UNSPEC address dumps, loss/debounce/resynchronization state, the subscribed route-netlink socket, and an opaque hard-budgeted driver registered with the existing daemon reactor only after the Capability Profile admits the mutation-capable startup path. Permanent observer failures invalidate the source and disable only observation. This is not yet the complete production Adapter described above because canonical link/route/rule fact codecs remain pending.
