...
- Added the separately invoked, digest-pinned xtables shell/AWK oracle. `cargo xtask xtables-oracle --check` verifies four raw IPv4/IPv6 apply/cleanup restore fixtures, while the explicit `--update` mode refreshes reviewed fixtures and their input/output metadata without changing the approved environment pins. The oracle binds the BusyBox `linux/amd64` OCI manifest `sha256:dfb66b2b3e6981fefa54fd2cd4faf662c35b4a4baeff48295a9409ddf3224c48` and `/bin/busybox` SHA-256 `c984eacc3b736fe1eeefe201f21b241932ef4c3c03fbb6869a4f156f32dd9716`, streams only one bounded five-file snapshot into private tmpfs, and applies inner/outer time and output bounds without mounting the host workspace. Generation has no container networking, live networking access, or restore execution and uses its own CI job rather than joining normal `cargo xtask ci`. This profile characterizes bounded cache inputs only; it does not cover configuration/kernel discovery, QUIC, PBR, forced cleanup, kernel acceptance, Android/Magisk parity, or establish a renderer, Generation, ownership, or activation path.
- Added a bounded, observation-only xtables restore parser/canonical codec in `flux-platform`. It preserves repeated table transactions, command/token order, duplicates, apply/cleanup opcode separation, per-transaction mangle cleanup ordering, explicit family context with current family-marker validation, resource usage, and a domain-separated byte digest while exposing no shell execution, restore process, Capture-policy-to-restore renderer, Generation, writer, ownership, prepared/active, coordinator, or activation path. Current-shaped synthetic fixtures exercise the closed grammar; the pinned raw shell fixtures are a separate byte-characterization gate, while renderer, kernel, and device parity remain later gates.
