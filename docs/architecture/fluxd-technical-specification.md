...
Open Phase 1 hardening gates are the remaining privileged namespace executor layers and Android adapter/qualification for the delivered functional traffic/loop-prevention transaction, an exact-device TUN single-owner and forced-death route-cleanup canary before removing the current TUN rejection, ancestor-safe `openat`/`openat2` traversal, bounded rotating Generation-correlated logs, pidfd/timerfd reactor integration, and real-device evidence on Android kernel 5.10.
