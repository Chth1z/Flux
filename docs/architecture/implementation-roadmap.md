...
2. Continue the positive local-OUTPUT producer on the delivered fail-closed TPROXY-only seam and
   ...
   qualified cgroup-eBPF authority. Use the delivered typed handoff from the real attempt context to
   carry the prebound strict `/proc` FD plus INET_DIAG session whose port is already in the request,
