...
The Sing-Box Supervisor foundation now binds Desired Engine state to SHA-256 identities for the exact binary, configuration, and optional BusyBox launcher. It reopens each bounded regular artifact once per launch, pins those verified descriptors through both `sing-box check` and `run`, and rehashes the same descriptors before readiness, preventing same-path replacement from changing the bytes checked or executed. Validation uses bounded diagnostics and an isolated process group whose descendants are killed and observed gone within a bounded cleanup deadline. Runtime launch has no shell layer, records PID plus `/proc` start ticks, rechecks that identity before every signal, and accepts pre-capture listener or TUN admission only when procfs proves that the resource is held by the owned child. Its reconciliation Interface is `reconcile(DesiredEngine, CaptureObservation)` plus an immutable snapshot. It enforces capture-detach ordering, bounded restart windows and exponential backoff, and a `Stopping` state that retains the child identity when post-`SIGKILL` reaping exceeds its deadline; no replacement may start until exit is observed. Owned-resource admission is deliberately not reported as the final post-capture functional proof: the atomic dispatcher handoff must add capture publication, loop-prevention/traffic verification, and fail-open rollback before the whole service reports `RUNNING`. The Supervisor is not yet the daemon's runtime owner, so that handoff must also ensure `scripts/core` and Rust can never control Sing-Box in the same daemon run. Ancestor-safe directory-relative opens, bounded rotating Generation-correlated runtime logs, and retained redacted diagnostic bundles are also assigned to that runtime handoff/hardening work; this foundation bounds in-memory tails and keeps raw tails out of public snapshots. Netlink, timerfd, pidfd/epoll child observation, active advanced probes, and BPF reactor sources remain assigned to their later phases and are not delivered by this milestone.
...
The legacy dispatcher remains the only networking-state writer. `fluxd` may supervise and observe it but must not directly write rules or routes yet.
