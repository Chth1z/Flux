...
Phase 1 `capture-verify` proves shell-owned structural evidence; the always-on owner bypass prevents the default self-capture omission but is not yet a synthetic end-to-end traffic or exact-process loop-prevention probe. Also deferred are ancestor-safe directory traversal with `openat`/`openat2`, long-term retention/rotation policy for generation logs, a pidfd/timerfd reactor, full process-tree containment, and real-device release evidence on the minimum Android 5.10 kernel.
