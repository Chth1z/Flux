...
Implementation checkpoint: the current substrate implements canonical combined link/address snapshots, strict private address and link decoders, loss/debounce/resynchronization state, the subscribed route-netlink socket, and an opaque hard-budgeted driver registered with the existing daemon reactor only after the Capability Profile admits the mutation-capable startup path. One socket issues `RTM_GETLINK` with `RTEXT_FILTER_SKIP_STATS`, waits for matching completion, then issues `AF_UNSPEC RTM_GETADDR`; only the second completion can replay transaction races and publish one `NetworkEpoch`. Any fault in either phase discards both partial classes and restarts at LINK. The link decoder preserves raw names and link kinds through the netlink wire bound, unknown flags/types/states, extended dump acknowledgements, and whole-datagram loss semantics; the observer preserves optional fields omitted by partial live notifications. Receive slots are 256 KiB with a 1 MiB default turn budget; 256 KiB is an operational bound rather than a protocol-wide maximum, so truncation remains a mandatory full-resync signal. Permanent observer failures invalidate the source and disable only observation.

The current route foundation defines canonical IPv4/IPv6 route prefixes, raw-preserving route properties, gateways, nexthops, paths, and records plus a strict private `RTM_NEWROUTE`/`RTM_DELROUTE` decoder. It validates table encoding, family-sized addresses, prefix host bits, multipath framing, deferred nested attributes, dump metadata, and loss signals while preserving `NLM_F_REPLACE`, named-nexthop IDs, unknown route values, and wire-order multipath weights. These records are not yet requested, reconciled, or published: omitted metrics, encapsulation, flow, and new-destination semantics make them a topology/selection projection rather than exact route state; NH-ID-only paths still require nexthop-object observation or a compatibility-mode gate; and live FIB identity/replacement remains deferred. Core constructors enforce semantic and family canonicality, not single-message rtnetlink encodability; future mutation encoders must enforce their exact attribute and message-size budgets.

The current rule foundation defines canonical IPv4/IPv6 policy-rule records plus a strict private `RTM_NEWRULE`/`RTM_DELRULE` decoder. It preserves raw action, origin-protocol, and rule flags; decodes table, priority, input/output interfaces, GOTO, fwmark/mask, tunnel ID, suppressors, L3MDEV, UID range, IP protocol, port ranges, and IPv4 flow/class ID; and validates the Linux 5.10 mandatory dump attributes, zero reserved header fields, family-sized prefixes, compact/extended table agreement, scalar widths and endianness, interface termination, range domains, repeatable padding, dump metadata, and loss signals. IPv6 `FRA_FLOW` follows the kernel's liberal ignored-input behavior and does not become a rule fact. Unknown actions and flags remain observable rather than being rejected.

These records are semantic selection projections: prefix host bits and fwmark bits outside the mask are normalized, so future mutation requires a separate raw kernel identity. Linux fib rules do not implement replacement through `NLM_F_REPLACE`, and equal-priority or duplicate rules are valid, so future inventory publication must retain dump order and multiplicity instead of using a priority map or record set. Rule facts are not yet requested, reconciled, or published. Exact `GETROUTE`/`GETRULE` transport, atomic route/rule publication, kernel identity, and native mutation remain pending, so the runtime inventory remains LINK/ADDRESS-only.

The route-netlink socket now also exposes exact `AF_UNSPEC RTM_GETROUTE` and `RTM_GETRULE` dump requests and nonblocking send methods. Each request is a 28-byte `nlmsghdr` plus zeroed 12-byte `rtmsg` or `fib_rule_hdr`, carries `NLM_F_REQUEST | NLM_F_DUMP`, uses a unique nonzero sequence, leaves the port ID for the kernel, and contains no filter attributes. Byte-exact endian tests and a sequential real-kernel smoke cover LINK→ADDRESS→ROUTE→RULE on one subscribed socket and one fixed receive ring. Only one dump may be active on that socket, so each phase must wait for matching `NLMSG_DONE` before sending the next. If best-effort strict dump checking is unavailable, a zero `GETROUTE` request may also return cloned exceptions; integration must retain the route decoder's validate-then-filter handling for `RTM_F_CLONED`. This is transport substrate only: the observer state machine still performs LINK→ADDRESS, does not reconcile route/rule events, and publishes only LINK/ADDRESS inventory.
